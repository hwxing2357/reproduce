...
from backbone.utils.moe_helpers import Softmax_Net, get_moe_outputs
...
class NPCL_MoE(NP_HEAD):
    def __init__(self, input_dim,
        ...
        logit_net_input_dim = input_dim+latent_dim if self.experts_in_gates else input_dim
        self.logit_net = Softmax_Net(logit_net_input_dim, self.experts_in_gates, 16, num_logit_layers=num_layers, num_experts=self.n_tasks, xavier_init=xavier_init)
    ...
    def moe_fusion(self, x_pred, z_experts_dict):
        z_experts = torch.stack(
            list(z_experts_dict.values()))
        z_experts_unsq = z_experts.unsqueeze(0).expand(x_pred.size(0), -1, -1, -1)
        if self.experts_in_gates:
            x_exp = x_pred.unsqueeze(1).unsqueeze(2).expand(-1, z_experts_unsq.size(1), z_experts_unsq.size(2), -1)
            x_z = torch.cat((x_exp, z_experts_unsq), dim=-1)
            alpha, y_hard = self.logit_net(x_z.mean(2), temperature=1, gumbel_max=True)
        else:
            alpha, y_hard = self.logit_net(x_pred, temperature=1, gumbel_max=True)
        return alpha, y_hard
    ...
    def forward(self, x_context_in, labels_context_in, x_target_in, labels_target_in=None,
                ...
                task_to_labels=None, top_k_decode=1, x_percent=10,  missing_dists = None) -> tuple:
        if phase_train:
            ...
            # to do, add check in get_local_latents() so that any missing task example in the batch is sampled from either missing_dist or randomly generated dist
            ...
            alpha, y_hard = self.moe_fusion(x_target_in, vs)
            ...
            return output, ((q_context, q_context_taskwise), (q_target, q_target_taskwise)), (alpha, y_hard)
        else:
            q_context, s_C, _ = self.latent_encoder(x_context_in, labels_context_in, context_task_labels)
            ...
            output = self.classifier(output_function)
            if len(vs) == 1:
                decoded_output = output.mean(0)
            else:
                decoded_output = get_moe_outputs(output, alpha, y_hard, hard=False, training=False, task_dist_ids=list(vs.keys()))
    ...
    def get_local_latents(self, q, clnp_stochasticity, forward_times, s, task_labels, phase_train=False, context=False,
                          task_to_labels=None, hierarchy=False, missing_dists=None):
        vs = None
        q_taskwise = {}
        if missing_dists is not None:
            q_taskwise = {task_id: dist for task_id, dist in missing_dists.items() if task_id not in task_labels}
        ...
        if clnp_stochasticity == 'all_global':
            for i in range(0, forward_times):
                ...
                # update with the cur batch statistics
                q_taskwise.update(self.encode_task(s, z, task_labels, task_to_labels))
            ...
            for i in range(0, forward_times):
                if missing_dists is not None:
                    # initialize with prev task distributions for prev tasks which were missing while sampling from the buffer
                    q_taskwise = {task_id: dist for task_id, dist in missing_dists.items() if task_id not in task_labels}
                zs = []
                ...
                for _ in torch.unique(task_labels):
                    ...
                    zs.append(z)
                zs = torch.stack(zs)
                q_taskwise.update(self.encode_task(s, zs, task_labels, task_to_labels))
