...
class Er(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs, m, cur_task=None, task_to_labels=None, global_step=None, num_total_iter=None, epoch_num=None):
        ...
        if self.args.np_type == '':
            ...
            context_labels_one_hot = F.one_hot(context_labels.view(-1).long(), num_classes=self.net.num_classes)
            # context_labels_one_hot = F.one_hot(context_labels.view(-1), num_classes=self.net.num_classes)

            labels_one_hot = F.one_hot(labels.view(-1).long(), num_classes=self.net.num_classes)
            ...
            loss = self.loss(outputs.mean(0), labels.to(torch.long))
            ...
            if not self.buffer.is_empty():
                ...
                buf_context_labels_one_hot = F.one_hot(buf_context_labels.view(-1).long(),
                                                       num_classes=self.np_head.num_classes)
                buf_labels_one_hot = F.one_hot(buf_labels.view(-1).long(), num_classes=self.np_head.num_classes)
                ...
                buf_logits_target_out, (buf_q_context, buf_q_target), (alpha, y_hard) = self.np_head(buf_context_outputs, context_labels,
                                                                               buf_feats,
                                                                               target_labels,
                                                                               forward_times=self.args.forward_times_train,
                ...
                loss += c_kld
                loss += self.loss(buf_logits_target_out.mean(0), buf_labels)
